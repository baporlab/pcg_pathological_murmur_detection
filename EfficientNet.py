...
"""
This code is EfficientNet for 1-D Convolution. This code was written by a team from Seoul National University who participated in PhysioNet Challenge 2021.

Followings are references. (Original EfficientNet and modified version by Seoul National University)
...
3. Suh, Jangwon, et al. "Learning ECG representations for multi-label classification of cardiac abnormalities." 2021 Computing in Cardiology (CinC). Vol. 48. IEEE, 2021.

"Final linear layer" was revised by Jae-Man Shin beacouse of computational costs.
nn.AdaptiveAvgPool1d(7) -> nn.AdaptiveAvgPool1d(1)
out_channels*7 -> out_channels*1
"""
...
class EfficientNet(torch.nn.Module):
    ...
    def __init__(self, num_leads, num_classes, model_num='efficientnet-b0', configs=None, info_for_test=None, blocks_args=None, global_params=None, include_top=True):
        super(EfficientNet, self).__init__()
        self.num_classes = num_classes
        override={'num_classes':num_classes, 'image_size': 4000, 'include_top': include_top}
        self.model_num = model_num
        blocks_args, global_params =get_model_params(model_num, override)
        ...
        self.last_dim = out_channels*1
        self.proj_dim = out_channels*1 # for projection head
        
        self.projection_head = nn.Sequential(
                                    nn.Linear(self.last_dim, self.proj_dim),
                                    nn.ReLU(),
                                    nn.Linear(self.proj_dim, self.proj_dim)
                                )
        self.cls1 = nn.Linear(self.last_dim+self.num_features, self._global_params.num_classes)
        self.cls2 = nn.Linear(self.last_dim, self._global_params.num_classes)
    ...
    def forward(self, inputs, features=None):
        ...
        if self._global_params.include_top:
            ...
            x = self._dropout(x)
            # x = self._fc(x)
            projected_reps = self.projection_head(x)
            if features != None:
                cls_reps = self.cls1(torch.cat((x, features), dim=1))
            else:
                cls_reps = self.cls2(x)
...
# Swish and MemoryEfficientSwish
